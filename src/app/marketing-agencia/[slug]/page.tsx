// ── /marketing-agencia/<slug> — страница тип „city“ (съдържание от de-os) ──
...
export async function generateStaticParams() {
	...
	return c.pages.filter((p) => p.type === "city").map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
	const { slug } = await params;
	const c = await getSiteContent();
	const page = c.pages.find((p) => p.type === "city" && p.slug === slug);
	if (!page) return {};
	const d = page.data;
...
export default async function Page({ params }: { params: Params }) {
	...
	const page = c.pages.find((p) => p.type === "city" && p.slug === slug);
	if (!page) notFound();
	return <ServicePageView page={page} catalog={catalog} content={c} crumbs={[{ name: "Градове", href: "/uslugi#city" }, { name: page.data.title, href: pagePath(page) }]} />;
}
